// Aviso de versión nueva en la parte superior de la pantalla de bienvenida.
...
class _UpdateBannerState extends State<UpdateBanner> {
  ...
  Future<void> _openRelease(String url) async {
    ...
    try {
      ...
      // Si el plugin no está disponible, `launchUrl` lanza PlatformException
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<UpdateStatus>(
      ...
      builder: (context, snapshot) {
        // Mientras no hay resultado no se pinta nada: un hueco que aparece y
        // desaparece empujaría el contenido mientras se lee.
        final result = snapshot.data;
        return switch (result) {
          UpdateAvailable(:final version, :final releaseUrl) => _available(
            context,
            version.toString(),
            releaseUrl,
          ),
  ...
  Widget _available(BuildContext context, String version, String releaseUrl) {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            Icon(Icons.download_for_offline, color: colors.onPrimaryContainer),
  ...
  Widget _failed(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Icon(Icons.info_outline, size: 14, color: colors.outline),
          const SizedBox(width: 8),
          Expanded(
            child: Text(
